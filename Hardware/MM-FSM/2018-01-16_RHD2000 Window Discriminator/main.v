...
//	wire 				DAC_en_5, DAC_en_6, DAC_en_7, DAC_en_8;
	...
	wire [15:0]		DAC_stop_max;
	reg  [7:0]		DAC_edge_type;
	wire [7:0]		DAC_in_window;
	reg  [7:0]	   DAC_state_status;
	reg  [7:0]		DAC_fsm_out;
   ...
   assign data_stream_8_en_in = 		ep14wirein[7];
	...
	// MM 1/15/2018 - FSM DISCRIMINATOR - START
	Max_8x16 max (
		.in1(DAC_stop_win_1),
		.in2(DAC_stop_win_2),
		.in3(DAC_stop_win_3),
	...
	assign TTL_out = TTL_out_mode ? {TTL_out_user[15:8], (DAC_fsm_mode ? DAC_fsm_out : DAC_thresh_out)} : TTL_out_user;
	// END
	...
	assign ep23wireout = 				DAC_fsm_mode ? {TTL_in[15:8], DAC_fsm_out} : TTL_in;
	// END UPDATE
   ...
   localparam
	...
	always @(posedge dataclk) begin
		if (reset) begin
			...
			DAC_state_status <= 8'b0000_0000;
			DAC_fsm_out <= 8'b0000_1000;
			...
			case (main_state)
				...
				ms_clk1_a: begin
					...
					if (channel == 0) begin				// grab TTL inputs, and grab current state of TTL outputs and manual DAC outputs
						// MM - UPDATE - WINDOW DISCRIMINATOR - 1/16/18
						data_stream_TTL_in <= DAC_fsm_mode ? {TTL_in[15:8], DAC_fsm_out} : TTL_in;
					...
					if (DAC_fsm_mode) begin
						case (fsm_window_state)
							...
							fsm_idle: begin
								DAC_fsm_counter <= 16'b0;
								DAC_fsm_out <= 8'b0000_0010;
								DAC_state_status <= (DAC_thresh_out ^ DAC_edge_type) | ((~DAC_in_window) | (~DAC_en));
								if ((&DAC_state_status) && (|DAC_en)) begin
						...
						DAC_state_status <= 8'b0000_0000;
						DAC_fsm_counter <= 16'b0;
						DAC_fsm_out <= 8'b0110_0000;
					end
					
					// MM 1/15/2018 - FSM DISCRIMINATOR - END
