...
MainWindow::MainWindow(int sampleRateIndex_, int stimStepIndex)
{
    ...
    cableLengthPortA = 3.0;
    cableLengthPortB = 3.0;
    cableLengthPortC = 3.0;
    cableLengthPortD = 3.0;
...
//    signalProcessor->setNotchFilterEnabled(notchFilterEnabled);
    ...
    lpEvtFilterFrequency = 5000.0;
    hpEvtFilterEnabled = false;
    lpEvtFilterEnabled = false;
    ...
    dacUpdateButton->setEnabled(true);
    ...
    if (!synthMode) {
        setDacThreshold1(dac1ThresholdSpinBox->value());
        setDacThreshold2(dac2ThresholdSpinBox->value());
        setDacThreshold3(dac3ThresholdSpinBox->value());
        setDacThreshold4(dac4ThresholdSpinBox->value());
        setDacThreshold5(dac5ThresholdSpinBox->value());
        setDacThreshold6(dac6ThresholdSpinBox->value());
        setDacThreshold7(dac7ThresholdSpinBox->value());
        setDacThreshold8(dac8ThresholdSpinBox->value());

        // MM - UPDATE - WINDOW DISCRIMINATOR - 1/16/2018
        setDac1WindowStart();
        setDac1WindowStop(dac1WindowStopSpinBox->value());
        setDac2WindowStart(dac2WindowStartSpinBox->value());
        setDac2WindowStop(dac2WindowStopSpinBox->value());
        setDac3WindowStart(dac3WindowStartSpinBox->value());
        setDac3WindowStop(dac3WindowStopSpinBox->value());
        setDac4WindowStart(dac4WindowStartSpinBox->value());
        setDac4WindowStop(dac4WindowStopSpinBox->value());
        setDac5WindowStart(dac5WindowStartSpinBox->value());
        setDac5WindowStop(dac5WindowStopSpinBox->value());
        setDac6WindowStart(dac6WindowStartSpinBox->value());
        setDac6WindowStop(dac6WindowStopSpinBox->value());
        setDac7WindowStart(dac7WindowStartSpinBox->value());
        setDac7WindowStop(dac7WindowStopSpinBox->value());
        setDac8WindowStart(dac8WindowStartSpinBox->value());
        setDac8WindowStop(dac8WindowStopSpinBox->value());

        setDac1ThresholdType(dac1ThresholdSpinBox->value());
        setDac2ThresholdType(dac2ThresholdSpinBox->value());
        setDac3ThresholdType(dac3ThresholdSpinBox->value());
        setDac4ThresholdType(dac4ThresholdSpinBox->value());
        setDac5ThresholdType(dac5ThresholdSpinBox->value());
        setDac6ThresholdType(dac6ThresholdSpinBox->value());
        setDac7ThresholdType(dac7ThresholdSpinBox->value());
        setDac8ThresholdType(dac8ThresholdSpinBox->value());
...
//    stimGroupBox->setLayout(stimLayout);
    ...
    numFramesComboBox->setCurrentIndex(8);
...
//            this, SLOT(changeNotchFilter(int)));
    ...
    // MM - UPDATE - IIT::NEW TABS FOR "EVENT" AND "DETECTION" MODULES - 5/24/2018

    QFrame *frameTab6 = new QFrame(); // " Event"
    QFrame *frameTab6Sub1 = new QFrame(); // "Discrete"
    QFrame *frameTab6Sub2 = new QFrame(); // "Wave"
    QFrame *frameTab7 = new QFrame(); // "Detection"
    ...
    dacUpdateButton = new QPushButton(tr("Update"));
    dacNoiseSuppressLayout->addWidget(dacUpdateButton);
    dacUpdateButton -> setEnabled(DetectionMethod);
    connect(dacUpdateButton, SIGNAL(clicked()),
            this, SLOT(dacUpdateParameters()));
    // END UPDATE
    ...
    tabWidget1->addTab(frameTab1, tr("Filters"));
    tabWidget1->addTab(frameTab2, tr("Impedance"));
    tabWidget1->addTab(frameTab3, tr("Audio/DAC"));
    tabWidget1->addTab(frameTab4, tr("Configure"));
    tabWidget1->addTab(frameTab5, tr("Display"));

    // MM - UPDATE - IIT:TAB MODULES - 5/24/2018

    tabWidget1->addTab(frameTab6, tr("Event"));
    tabWidget1->addTab(frameTab7, tr("Detection"));

    QTabWidget *tabSubWidget1 = new QTabWidget();
    tabSubWidget1->addTab(frameTab6Sub1,tr("Discrete"));
    tabSubWidget1->addTab(frameTab6Sub2,tr("Wave"));
    // MM - END UPDATE


    // MM - UPDATE - IIT: EVENT TAB - 5/24/2018
    QVBoxLayout *evtLayout = new QVBoxLayout;
    QHBoxLayout *discreteLayout = new QHBoxLayout;
    QVBoxLayout *discreteControlLayout = new QVBoxLayout;
    QHBoxLayout *discretePlotLayout = new QHBoxLayout;
    QHBoxLayout *waveLayout = new QHBoxLayout;
    QVBoxLayout *waveControlLayout = new QVBoxLayout;
    QHBoxLayout *wavePlotLayout = new QHBoxLayout;


    // Things for "DISCRETE" sub-tab
    QGroupBox *discreteControlBox = new QGroupBox(tr("Controls"));
    QGroupBox *discretePlotBox = new QGroupBox(tr("Plots"));
    QHBoxLayout *discreteEpochPreLayout = new QHBoxLayout;
    QHBoxLayout *discreteEpochPostLayout = new QHBoxLayout;
    QVBoxLayout *discreteEpochLayout = new QVBoxLayout;
    QCheckBox *discreteEnableCheckBox = new QCheckBox(tr("Enable"));
    connect(discreteEnableCheckBox, SIGNAL(clicked(bool)),
            this, SLOT(setDiscreteEventPlotState(bool)));
    QSpinBox *discretePreSpinBox = new QSpinBox();
    discretePreSpinBox->setRange(-1000,1000);
    discretePreSpinBox->setSingleStep(25);
    discretePreSpinBox->setValue(-50);
    QSpinBox *discretePostSpinBox = new QSpinBox();
    discretePostSpinBox->setRange(-1000,1000);
    discretePostSpinBox->setSingleStep(25);
    discretePostSpinBox->setValue(-50);
    QGroupBox *discreteEpochBox = new QGroupBox(tr("Timing"));
    QComboBox *discretePlotTypeComboBox = new QComboBox();
    discretePlotTypeComboBox->addItem(tr("Count"));
    discretePlotTypeComboBox->addItem(tr("Raster"));
    discretePlotTypeComboBox->addItem(tr("PSTH"));
    discretePlotTypeComboBox->setCurrentIndex(0);

    QStringList triggerSourceList;
    triggerSourceList << "DIGITAL IN 1" << "DIGITAL IN 2" << "DIGITAL IN 3" << "DIGITAL IN 4" << "DIGITAL IN 5"
    ...
    QStringList nEventList;
    nEventList << "10" << "20" << "30" << "rolling";

    QComboBox *discretePlotTriggerComboBox = new QComboBox();
    discretePlotTriggerComboBox->addItems(triggerSourceList);
    discretePlotTriggerComboBox->setCurrentIndex(0);


    discreteEpochPreLayout->addWidget(new QLabel(tr("Pre (ms):")));
    discreteEpochPreLayout->addWidget(discretePreSpinBox);

    discreteEpochPostLayout->addWidget(new QLabel(tr("Post (ms):")));
    discreteEpochPostLayout->addWidget(discretePostSpinBox);

    discreteEpochLayout->addLayout(discreteEpochPreLayout);
    discreteEpochLayout->addLayout(discreteEpochPostLayout);
    discreteEpochBox->setLayout(discreteEpochLayout);

    QComboBox *discretePlotNEventComboBox = new QComboBox();
    discretePlotNEventComboBox->addItems(nEventList);
    discretePlotNEventComboBox->setCurrentIndex(0);

    discreteControlLayout->addStretch(1);
    discreteControlLayout->addWidget(discreteEnableCheckBox);
    discreteControlLayout->addStretch(1);
    discreteControlLayout->addWidget(new QLabel(tr("Plot Type")));
    discreteControlLayout->addWidget(discretePlotTypeComboBox);
    discreteControlLayout->addStretch(1);
    discreteControlLayout->addWidget(new QLabel(tr("Trigger")));
    discreteControlLayout->addWidget(discretePlotTriggerComboBox);
    discreteControlLayout->addStretch(1);
    discreteControlLayout->addWidget(discreteEpochBox);
    discreteControlLayout->addStretch(1);
    discreteControlLayout->addWidget(new QLabel(tr("# Trig Events")));
    discreteControlLayout->addWidget(discretePlotNEventComboBox);
    discreteControlLayout->addStretch(1);

    discretePlotLayout->addStretch(1);

    discreteControlBox->setLayout(discreteControlLayout);
    discretePlotBox->setLayout(discretePlotLayout);

    discreteLayout->addWidget(discreteControlBox);
    discreteLayout->addSpacing(1);
    discreteLayout->addWidget(discretePlotBox);

    frameTab6Sub1->setLayout(discreteLayout);

    // Things for "WAVE" sub-tab
    QGroupBox *waveControlBox = new QGroupBox(tr("Controls"));
    QGroupBox *wavePlotBox = new QGroupBox(tr("Plots"));
    QHBoxLayout *waveEpochPreLayout = new QHBoxLayout;
    QHBoxLayout *waveEpochPostLayout = new QHBoxLayout;
    QVBoxLayout *waveEpochLayout = new QVBoxLayout;
    QCheckBox *waveEnableCheckBox = new QCheckBox(tr("Enable"));
    connect(waveEnableCheckBox, SIGNAL(clicked(bool)),
            this, SLOT(setWaveEventPlotState(bool)));
    QSpinBox *wavePreSpinBox = new QSpinBox();
    wavePreSpinBox->setRange(-1000,1000);
    wavePreSpinBox->setSingleStep(25);
    wavePreSpinBox->setValue(-50);
    QSpinBox *wavePostSpinBox = new QSpinBox();
    wavePostSpinBox->setRange(-1000,1000);
    wavePostSpinBox->setSingleStep(25);
    wavePostSpinBox->setValue(-50);
    QGroupBox *waveEpochBox = new QGroupBox(tr("Timing"));
    QComboBox *wavePlotTypeComboBox = new QComboBox();
    wavePlotTypeComboBox->addItem(tr("LFP"));
    wavePlotTypeComboBox->addItem(tr("MUA"));
    wavePlotTypeComboBox->addItem(tr("EMG"));
    wavePlotTypeComboBox->setCurrentIndex(0);

    QComboBox *wavePlotTriggerComboBox = new QComboBox();
    wavePlotTriggerComboBox->addItems(triggerSourceList);
    wavePlotTriggerComboBox->setCurrentIndex(0);

    waveEpochPreLayout->addWidget(new QLabel(tr("Pre (ms):")));
    waveEpochPreLayout->addWidget(wavePreSpinBox);

    waveEpochPostLayout->addWidget(new QLabel(tr("Post (ms):")));
    waveEpochPostLayout->addWidget(wavePostSpinBox);

    waveEpochLayout->addLayout(waveEpochPreLayout);
    waveEpochLayout->addLayout(waveEpochPostLayout);
    waveEpochBox->setLayout(waveEpochLayout);

    QComboBox *wavePlotNEventComboBox = new QComboBox();
    wavePlotNEventComboBox->addItems(nEventList);
    wavePlotNEventComboBox->setCurrentIndex(0);

    waveControlLayout->addStretch(1);
    waveControlLayout->addWidget(waveEnableCheckBox);
    waveControlLayout->addStretch(1);
    waveControlLayout->addWidget(new QLabel(tr("Plot Type")));
    waveControlLayout->addWidget(wavePlotTypeComboBox);
    waveControlLayout->addStretch(1);
    waveControlLayout->addWidget(new QLabel(tr("Trigger")));
    waveControlLayout->addWidget(wavePlotTriggerComboBox);
    waveControlLayout->addStretch(1);
    waveControlLayout->addWidget(waveEpochBox);
    waveControlLayout->addStretch(1);
    waveControlLayout->addWidget(new QLabel(tr("# Trig Events")));
    waveControlLayout->addWidget(wavePlotNEventComboBox);
    waveControlLayout->addStretch(1);

    wavePlotLayout->addStretch(1);

    waveControlBox->setLayout(waveControlLayout);
    wavePlotBox->setLayout(wavePlotLayout);

    waveLayout->addWidget(waveControlBox);
    waveLayout->addSpacing(1);
    waveLayout->addWidget(wavePlotBox);

    frameTab6Sub2->setLayout(waveLayout);
    frameTab6->setLayout(evtLayout);
    evtLayout->addWidget(tabSubWidget1);
    // MM - END UPDATE
...
//    bandwidthLayout->addWidget(lowerSettleBandwidthLabel);
    ...
    QGroupBox *dacFilterGroupBox = new QGroupBox(tr("DAC Filter"));
    dacFilterGroupBox->setLayout(offchipFilterLayout);

    // MM - UPDATE - IIT:NEW HARDWARE FILTER MODULE - 5/25/2018
    // enables
    QButtonGroup *evtFilterTypeButtonGroup = new QButtonGroup();
    hpEvtFilterButton = new QRadioButton(tr("Hardware Event Stream High-Pass Filter (1-pole butter)"));
    lpEvtFilterButton = new QRadioButton(tr("Hardware Event Stream Low-Pass Filter (1-pole butter)"));
    evtFilterTypeButtonGroup->addButton(hpEvtFilterButton,0);
    evtFilterTypeButtonGroup->addButton(lpEvtFilterButton,1);
    hpEvtFilterButton->setChecked(true);

    connect(evtFilterTypeButtonGroup, SIGNAL(buttonClicked(int)),
            this, SLOT(evtFilterSelected(int)));

    hpEvtFilterLineEdit = new QLineEdit(QString::number(hpEvtFilterFrequency, 'f', 0));
    hpEvtFilterLineEdit->setValidator(new QDoubleValidator(0.01, 9999.99, 2, this));
    connect(hpEvtFilterLineEdit, SIGNAL(textChanged(const QString &)),
            this, SLOT(hpEvtFilterLineEditChanged()));

    lpEvtFilterLineEdit = new QLineEdit(QString::number(lpEvtFilterFrequency, 'f', 0));
    lpEvtFilterLineEdit->setValidator(new QDoubleValidator(0.01, 9999.99, 2, this));
    connect(lpEvtFilterLineEdit, SIGNAL(textChanged(const QString &)),
            this, SLOT(lpEvtFilterLineEditChanged()));

    QHBoxLayout *hpEvtFilterLayout = new QHBoxLayout;
    hpEvtFilterLayout->addWidget(hpEvtFilterButton);
    hpEvtFilterLayout->addWidget(hpEvtFilterLineEdit);
    hpEvtFilterLayout->addWidget(new QLabel(tr("Hz")));
    hpEvtFilterLayout->addStretch(1);

    QHBoxLayout *lpEvtFilterLayout = new QHBoxLayout;
    lpEvtFilterLayout->addWidget(lpEvtFilterButton);
    lpEvtFilterLayout->addWidget(lpEvtFilterLineEdit);
    lpEvtFilterLayout->addWidget(new QLabel(tr("Hz")));
    lpEvtFilterLayout->addStretch(1);

    QVBoxLayout *evtFilterLayout = new QVBoxLayout;
    evtFilterLayout->addLayout(hpEvtFilterLayout);
    evtFilterLayout->addLayout(lpEvtFilterLayout);

    QGroupBox *evtFilterGroupBox = new QGroupBox(tr("Hardware Event Filters"));
    evtFilterGroupBox->setLayout(evtFilterLayout);
    ...
    freqLayout->addStretch(1);
    freqLayout->addWidget(dacFilterGroupBox);
    freqLayout->addStretch(1);
    freqLayout->addWidget(evtFilterGroupBox);
    freqLayout->addStretch(1);
    // MM - END UPDATE
...
void MainWindow::runInterfaceBoard()
{
    ...
    // SB update - to load data from file
    QString data_to_replay_name;
    vector<double> vector_data;
    ifstream inputFile;
    int fileSize;

    if (synthMode) {
        data_to_replay_name = QFileDialog::getOpenFileName(this,
                                                           tr("Open Text file"), "/Users/stefanobuccelli/Desktop/Intan_stuff", tr("Text Files (*.txt)"));
        //cout << int(data_to_replay_name.size()) << endl;
        if(!data_to_replay_name.isEmpty()){
            inputFile.open(data_to_replay_name.toStdString(),ios::binary | ios::ate);
...
//            }
    ...
    dacUpdateButton->setEnabled(false);
    ...
    while (running) {
        ...
        if (newDataReady) {
            ...
            if (synthMode) {
                ...
                signalProcessor->allocateMemory(1); // try to reset every time to avoid wired filter effects
                fifoPercentageFull = 0.0;
                count_new_data_ready++;
...
//                                                           *saveStream, saveFormat, saveTtlOut, saveDcAmps, referenceSource);
                ...
                // SB update 23-05-2018 Replay data
                // reset counter to avoid memory errors
                int buffer_size_replay=count_new_data_ready*CHANNELS_PER_STREAM*SAMPLES_PER_DATA_BLOCK*numUsbBlocksToRead;
                // cout << inputFile.tellg() << endl;
                if(!(buffer_size_replay<=(fileSize/8)-buffer_size_replay)){ // 8 because it's double so 8 bytes for each element?
                    inputFile.seekg(0, ios::beg);
                    count_new_data_ready=0;
                    cout << "reset ifstream" << endl;
                }
                signalProcessor->loadReplayData(numUsbBlocksToRead,
                                                boardSampleRate, recording,
                                                *saveStream, saveFormat, saveTtlOut, saveDcAmps, referenceSource,inputFile);


//                if(!(vector_data.size()==0)){
//                    if(!(vector_data.size()>(count_new_data_ready*CHANNELS_PER_STREAM*SAMPLES_PER_DATA_BLOCK*numUsbBlocksToRead))){
//                        count_new_data_ready=0;
//                    }
//                    signalProcessor->loadReplayData(numUsbBlocksToRead,
//                                                    boardSampleRate, recording,
//                                                    *saveStream, saveFormat, saveTtlOut, saveDcAmps, referenceSource,vector_data, count_new_data_ready);
//                }
            } else {
                ...
                if (fifoNearlyFull > 2) {
                    ...
                    QMessageBox::critical(this, tr("USB Buffer Overrun Error"),
                                          tr("Recording was stopped because the USB FIFO buffer on the interface "
                                             ...
                                             "<p>Try lowering the sample rate, or reducing "
...
void MainWindow::spikeScope()
{
    ...
    spikeScopeDialog->setDACenable(0,dacEnabled[0]);
    spikeScopeDialog->setDACenable(1,dacEnabled[1]);
    spikeScopeDialog->setDACenable(2,dacEnabled[2]);
    spikeScopeDialog->setDACenable(3,dacEnabled[3]);
    spikeScopeDialog->setDACenable(4,dacEnabled[4]);
    spikeScopeDialog->setDACenable(5,dacEnabled[5]);
    spikeScopeDialog->setDACenable(6,dacEnabled[6]);
    spikeScopeDialog->setDACenable(7,dacEnabled[7]);

    spikeScopeDialog->setDACThreshold(0,dac1ThresholdSpinBox->value());
    spikeScopeDialog->setDACThreshold(1,dac2ThresholdSpinBox->value());
    spikeScopeDialog->setDACThreshold(2,dac3ThresholdSpinBox->value());
    spikeScopeDialog->setDACThreshold(3,dac4ThresholdSpinBox->value());
    spikeScopeDialog->setDACThreshold(4,dac5ThresholdSpinBox->value());
    spikeScopeDialog->setDACThreshold(5,dac6ThresholdSpinBox->value());
    spikeScopeDialog->setDACThreshold(6,dac7ThresholdSpinBox->value());
    spikeScopeDialog->setDACThreshold(7,dac8ThresholdSpinBox->value());

    spikeScopeDialog->setDACTriggerType(0,dac1TypeComboBox->currentIndex());
    spikeScopeDialog->setDACTriggerType(1,dac2TypeComboBox->currentIndex());
    spikeScopeDialog->setDACTriggerType(2,dac3TypeComboBox->currentIndex());
    spikeScopeDialog->setDACTriggerType(3,dac4TypeComboBox->currentIndex());
    spikeScopeDialog->setDACTriggerType(4,dac5TypeComboBox->currentIndex());
    spikeScopeDialog->setDACTriggerType(5,dac6TypeComboBox->currentIndex());
    spikeScopeDialog->setDACTriggerType(6,dac7TypeComboBox->currentIndex());
    spikeScopeDialog->setDACTriggerType(7,dac8TypeComboBox->currentIndex());

    spikeScopeDialog->setDACWindowStart(0,dac1WindowStartSpinBox->value());
    spikeScopeDialog->setDACWindowStart(1,dac2WindowStartSpinBox->value());
    spikeScopeDialog->setDACWindowStart(2,dac3WindowStartSpinBox->value());
    spikeScopeDialog->setDACWindowStart(3,dac4WindowStartSpinBox->value());
    spikeScopeDialog->setDACWindowStart(4,dac5WindowStartSpinBox->value());
    spikeScopeDialog->setDACWindowStart(5,dac6WindowStartSpinBox->value());
    spikeScopeDialog->setDACWindowStart(6,dac7WindowStartSpinBox->value());
    spikeScopeDialog->setDACWindowStart(7,dac8WindowStartSpinBox->value());

    spikeScopeDialog->setDACWindowStop(0,dac1WindowStopSpinBox->value());
    spikeScopeDialog->setDACWindowStop(1,dac2WindowStopSpinBox->value());
    spikeScopeDialog->setDACWindowStop(2,dac3WindowStopSpinBox->value());
    spikeScopeDialog->setDACWindowStop(3,dac4WindowStopSpinBox->value());
    spikeScopeDialog->setDACWindowStop(4,dac5WindowStopSpinBox->value());
    spikeScopeDialog->setDACWindowStop(5,dac6WindowStopSpinBox->value());
    spikeScopeDialog->setDACWindowStop(6,dac7WindowStopSpinBox->value());
    spikeScopeDialog->setDACWindowStop(7,dac8WindowStopSpinBox->value());

    spikeScopeDialog->setDetectionMode(DetectionMethod);
    spikeScopeDialog->setCurrentChannel(0);

    wavePlot->setFocus();

    bool AllowUpdate = DetectionMethod && spikeScopeDialog;
    dacUpdateButton->setEnabled(AllowUpdate);
}
...
void MainWindow::changeDetectionMethod(bool enable)
{
    ...
    bool AllowUpdate = enable && spikeScopeDialog;
    dacUpdateButton->setEnabled(AllowUpdate);
}

// Click this after changing windows in window scope to upload parameters
void MainWindow::dacUpdateParameters()
{
    if (spikeScopeDialog) {
        dacEnabled = spikeScopeDialog->wEnable;

        dac1ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[0]);
        emit setDacThreshold1(dac1ThresholdSpinBox->value());
        QThread::msleep(1);
        dac1TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[0]);
        emit setDac1ThresholdType(dac1TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac1WindowStartSpinBox->setValue(spikeScopeDialog->wStart[0]);
        dac1WindowStopSpinBox->setValue(spikeScopeDialog->wStop[0]);
        emit setDac1WindowStop(dac1WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton1->setChecked(true);
        dacEnable(dacEnabled[0]);
        if (dacEnabled[0]) {
            dacSetChannel();
        }
        QThread::msleep(1);

        dac2ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[1]);
        emit setDacThreshold2(dac2ThresholdSpinBox->value());
        QThread::msleep(1);
        dac2TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[1]);
        emit setDac2ThresholdType(dac2TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac2WindowStartSpinBox->setValue(spikeScopeDialog->wStart[1]);
        emit setDac2WindowStart(dac2WindowStartSpinBox->value());
        QThread::msleep(1);
        dac2WindowStopSpinBox->setValue(spikeScopeDialog->wStop[1]);
        emit setDac2WindowStop(dac2WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton2->setChecked(true);
        dacEnable(dacEnabled[1]);
        if (dacEnabled[1]) {
            dacSetChannel();
        }
        QThread::msleep(1);

        dac3ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[2]);
        emit setDacThreshold3(dac3ThresholdSpinBox->value());
        QThread::msleep(1);
        dac3TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[2]);
        emit setDac3ThresholdType(dac3TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac3WindowStartSpinBox->setValue(spikeScopeDialog->wStart[2]);
        emit setDac3WindowStart(dac3WindowStartSpinBox->value());
        QThread::msleep(1);
        dac3WindowStopSpinBox->setValue(spikeScopeDialog->wStop[2]);
        emit setDac3WindowStop(dac3WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton3->setChecked(true);
        dacEnable(dacEnabled[2]);
        if (dacEnabled[2]) {
            dacSetChannel();
        }
        QThread::msleep(1);

        dac4ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[3]);
        emit setDacThreshold4(dac4ThresholdSpinBox->value());
        QThread::msleep(1);
        dac4TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[3]);
        emit setDac4ThresholdType(dac4TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac4WindowStartSpinBox->setValue(spikeScopeDialog->wStart[3]);
        emit setDac4WindowStart(dac4WindowStartSpinBox->value());
        QThread::msleep(1);
        dac4WindowStopSpinBox->setValue(spikeScopeDialog->wStop[3]);
        emit setDac4WindowStop(dac4WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton4->setChecked(true);
        dacEnable(dacEnabled[3]);
        if (dacEnabled[3]) {
            dacSetChannel();
        }
        QThread::msleep(1);

        dac5ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[4]);
        emit setDacThreshold5(dac5ThresholdSpinBox->value());
        QThread::msleep(1);
        dac5TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[4]);
        emit setDac5ThresholdType(dac5TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac5WindowStartSpinBox->setValue(spikeScopeDialog->wStart[4]);
        emit setDac5WindowStart(dac5WindowStartSpinBox->value());
        QThread::msleep(1);
        dac5WindowStopSpinBox->setValue(spikeScopeDialog->wStop[4]);
        emit setDac5WindowStop(dac5WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton5->setChecked(true);
        dacEnable(dacEnabled[4]);
        if (dacEnabled[4]) {
            dacSetChannel();
        }
        QThread::msleep(1);

        dac6ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[5]);
        emit setDacThreshold6(dac6ThresholdSpinBox->value());
        QThread::msleep(1);
        dac6TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[5]);
        emit setDac6ThresholdType(dac6TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac6WindowStartSpinBox->setValue(spikeScopeDialog->wStart[5]);
        emit setDac6WindowStart(dac6WindowStartSpinBox->value());
        QThread::msleep(1);
        dac6WindowStopSpinBox->setValue(spikeScopeDialog->wStop[5]);
        emit setDac6WindowStop(dac6WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton6->setChecked(true);
        dacEnable(dacEnabled[5]);
        if (dacEnabled[5]) {
            dacSetChannel();
        }
        QThread::msleep(1);

        dac7ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[6]);
        emit setDacThreshold7(dac7ThresholdSpinBox->value());
        QThread::msleep(1);
        dac7TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[6]);
        emit setDac7ThresholdType(dac7TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac7WindowStartSpinBox->setValue(spikeScopeDialog->wStart[6]);
        emit setDac7WindowStart(dac7WindowStartSpinBox->value());
        QThread::msleep(1);
        dac7WindowStopSpinBox->setValue(spikeScopeDialog->wStop[6]);
        emit setDac7WindowStop(dac7WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton7->setChecked(true);
        dacEnable(dacEnabled[6]);
        if (dacEnabled[6]) {
            dacSetChannel();
        }
        QThread::msleep(1);

        dac8ThresholdSpinBox->setValue(spikeScopeDialog->wThresh[7]);
        emit setDacThreshold8(dac8ThresholdSpinBox->value());
        QThread::msleep(1);
        dac8TypeComboBox->setCurrentIndex(spikeScopeDialog->wType[7]);
        emit setDac8ThresholdType(dac8TypeComboBox->currentIndex());
        QThread::msleep(1);
        dac8WindowStartSpinBox->setValue(spikeScopeDialog->wStart[7]);
        emit setDac8WindowStart(dac8WindowStartSpinBox->value());
        QThread::msleep(1);
        dac8WindowStopSpinBox->setValue(spikeScopeDialog->wStop[7]);
        emit setDac8WindowStop(dac8WindowStopSpinBox->value());
        QThread::msleep(1);
        dacButton8->setChecked(true);
        dacEnable(dacEnabled[7]);
        if (dacEnabled[7]) {
            dacSetChannel();
        }
        QThread::msleep(1);
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(0, sample);
        windowMax = evalBoard->setMaxWindowStop(0, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
}
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(1, sample);
        windowMax = evalBoard->setMaxWindowStop(1, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(2, sample);
        windowMax = evalBoard->setMaxWindowStop(2, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(3, sample);
        windowMax = evalBoard->setMaxWindowStop(3, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(4, sample);
        windowMax = evalBoard->setMaxWindowStop(4, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(5, sample);
        windowMax = evalBoard->setMaxWindowStop(5, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(6, sample);
        windowMax = evalBoard->setMaxWindowStop(6, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
...
{
    if (!synthMode) {
        evalBoard->setDacWindowStop(7, sample);
        windowMax = evalBoard->setMaxWindowStop(7, sample);
        if (spikeScopeDialog) {
            spikeScopeDialog->setWMax(windowMax);
        }
    }
...
void MainWindow::setDiscreteEventPlotState(bool enable)
{
    enableDiscreteEvents = enable;
}

void MainWindow::setWaveEventPlotState(bool enable)
{
    enableWaveEvents = enable;
}
// MM - END UPDATE

// MM - UPDATE - IIT:EVENTS STREAM FILTERS - 5/25/2018
void MainWindow::hpEvtFilterLineEditChanged()
{
    sethpEvtFilterCutoff(hpEvtFilterLineEdit->text().toDouble());
}

// Update software/FPGA high-pass filter cutoff frequency.
void MainWindow::sethpEvtFilterCutoff(double cutoff)
{
    hpEvtFilterFrequency = cutoff;
//    signalProcessor->setHighpassFilter(cutoff , boardSampleRate);
    if (!synthMode) {
        // This will be where the pipeout goes to the FPGA
//        evalBoard->setDacHighpassFilter(cutoff);
    }
}

void MainWindow::lpEvtFilterLineEditChanged()
{
    setlpEvtFilterCutoff(lpEvtFilterLineEdit->text().toDouble());
}

// Update software/FPGA high-pass filter cutoff frequency.
void MainWindow::setlpEvtFilterCutoff(double cutoff)
{
    lpEvtFilterFrequency = cutoff;
...
void MainWindow::enablehpEvtFilter(bool enable)
{
    hpEvtFilterEnabled = enable;
}

void MainWindow::enablelpEvtFilter(bool enable)
{
    lpEvtFilterEnabled = enable;
}

void MainWindow::evtFilterSelected(int buttonIndex)
{
    switch (buttonIndex) {
    case 0: // Top (HIGHPASS)
        enablehpEvtFilter(true);
        sethpEvtFilterCutoff(hpEvtFilterFrequency);
        break;
    case 1: // Bottom (LOWPASS)
        enablelpEvtFilter(true);
        setlpEvtFilterCutoff(lpEvtFilterFrequency);
        break;
    default:
        enablehpEvtFilter(true);
...
// set properties depending on which type of DAC filter is selected
void MainWindow::dacFilterSelected(int index)
{
    switch (index) {
    case 0: // NO FILTER
