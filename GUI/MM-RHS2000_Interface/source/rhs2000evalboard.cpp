...
// This is useful when using the low-latency FPGA thresholds to detect spikes
// and produce digital pulses on the TTL outputs, for example.
void Rhs2000EvalBoard::setEvtHighpassFilter(double cutoff)
{
    lock_guard<mutex> lockOk(okMutex);
...
// and produce digital pulses on the TTL outputs, for example.
void Rhs2000EvalBoard::setEvtLowpassFilter(double cutoff)
{
    lock_guard<mutex> lockOk(okMutex);
...
int Rhs2000EvalBoard::setMaxWindowStop(int dacChannel, int sample)
{
    ...
    if (dacChannel == maxWindowDAC) {
        if (sample < maxWindowStop) { // Not familiar with C++ syntax, so doing crude implementation
            int tree_1_1v, tree_1_2v, tree_1_3v, tree_1_4v;
            int tree_2_1v, tree_2_2v;

            int tree_1_1c, tree_1_2c, tree_1_3c, tree_1_4c;
            int tree_2_1c, tree_2_2c;

            if (wEnable[0] && wEnable[1]) {
                if (W1_stop >= W2_stop) {
                    tree_1_1v = W1_stop;
                    tree_1_1c = 1;
                } else {
            ...
            if (tree_2_1v >= tree_2_2v) {
                maxWindowStop = tree_2_1v;
                maxWindowDAC = tree_2_1c;
            } else {
                maxWindowStop = tree_2_2v;
                maxWindowDAC = tree_2_2c;
            }
