...
void SignalProcessor::allocateMemory(int numStreams)
{
    ...
    allocateDoubleArray2D(filterState, numStreams, CHANNELS_PER_STREAM);
    ...
    fillZerosDoubleArray2D(filterState);
...
// SB update 23 - 05 - 2018
// This function behaves similarly to loadAmplifierData, but performs a replay of data.

// Returns number of bytes written to binary datastream out if saveToDisk == true.
int SignalProcessor::loadReplayData(int numBlocks, double sampleRate,
                                       ...
                                       ifstream &fileName)
{
    ...
    //fileName.seekg(0);
        ...
        // Put data from vector_data to amplifierPreFilterFast in the right way
        for (t = 0; t < SAMPLES_PER_DATA_BLOCK * numBlocks; ++t) {
            for (stream = 0; stream < numDataStreams; ++stream) {
                for (channel = 0; channel < CHANNELS_PER_STREAM; ++channel) {
//                    amplifierPreFilterFast[fastIndex(stream, channel, t)] = vector_data[count_new_data_ready*(channel+(16*(t+1)))];
                        fileName >> value;
                        amplifierPreFilterFast[fastIndex(stream, channel, t)]=value;
                }
            }
        }

// from now on it's equal to loadSyntheticData (need to change it if we need other data like DAC etc in replay mode)
    if (referenceSource.softwareMode) {
        for (stream = 0; stream < numDataStreams; ++stream) {
            for (channel = 0; channel < CHANNELS_PER_STREAM; ++channel) {
                                 ...
                                 const QVector<QVector<bool> > &channelVisible)
{
    ...
    // MM - UPDATE - IIT:FILTERING EVENT STREAMS - 5/25/2018
