...
void SignalGroup::addBoardAdcChannel(int nativeChannelNumber)
{
    ...
    nativeChannelName = prefix + "-" + QString("%1").arg(nativeChannelNumber + 1, 1, 10, QChar('0'));
    customChannelName = nativeChannelName;
...
// Add new USB interface board DAC channel to this signal group.
void SignalGroup::addBoardDacChannel(int nativeChannelNumber)
{
    QString nativeChannelName;
    QString customChannelName;
    ...
    SignalChannel *newChannel = new SignalChannel(customChannelName, nativeChannelName,
                                                  nativeChannelNumber, BoardDacSignal,
                                                  nativeChannelNumber, 0, this);
    channel.append(*newChannel);
    updateAlphabeticalOrder();
}
...
void SignalGroup::addBoardDigInChannel(int nativeChannelNumber)
{
    ...
    nativeChannelName = prefix + "-" + QString("%1").arg(nativeChannelNumber + 1, 2, 10, QChar('0'));
    customChannelName = nativeChannelName;
...
void SignalGroup::writeToXml(QXmlStreamWriter &xml)
{
    for (int i = 0; i < channel.size(); ++i) {
        channel[i].writeToXml(xml);
    }
}

void SignalGroup::disableAllStim()
{
    for (int i = 0; i < channel.size(); ++i) {
        channel[i].stimParameters->enabled = false;
    }
}
