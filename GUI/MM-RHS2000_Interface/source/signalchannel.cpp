...
// (e.g., an amplifier channel on a particular RHS2000 chip, a digital
...
SignalChannel::SignalChannel()
{
    ...
    stimParameters = new StimParameters();
}
...
QDataStream& operator<<(QDataStream &outStream, const SignalChannel &a)
{
    ...
    outStream << (qint16) a.commandStream;
...
QDataStream& operator>>(QDataStream &inStream, SignalChannel &a)
{
    ...
    inStream >> tempQint16;
    a.commandStream = (int) tempQint16;
    inStream >> tempQint16;
...
void SignalChannel::writeToXml(QXmlStreamWriter &xml)
{
    if (stimParameters->enabled) {
        xml.writeStartElement("channel");
        xml.writeAttribute("name", nativeChannelName);
        stimParameters->writeXml(xml, signalType);
        xml.writeEndElement();
    }
}
